...
class ClientController extends AbstractActionController
{
   ...
   /**
    ...
    public function indexAction(){
		...
		$clientList = $service->getClientMapper()->fetchAllByAgency($this->ZfcUserAuthentication()->getIdentity()->getId());
    ...
    public function addAction(){
		...
		if($request->isPost()){			
			...
			$userInfo     = array( 
								   ...
								   'state'					=> 1,
								   'type'					=> 'client',
								   'parent'					=> $this->ZfcUserAuthentication()->getIdentity()->getId()
								 );
			...
			if($client){
		   	 	return $this->redirect()->toUrl($this->url()->fromRoute('client'));
		    }
	...
	public function viewAction(){
		...
		$reports_service  = $this->getServiceLocator()->get('jimmybase_reports_service');
		$form    		  = $this->getClientRegisterForm();
		...
		$reportsList   = $reports_service->getMapper()->findByUserId($id);
		...
		$viewModel = new ViewModel(array(
			...
			'client_mapper'   => $client_service->getClientMapper(),
			'agency_mapper'   => $agency_service->getMapper(),
		));
	...
	public function createReportAction(){
		...
		$metrics_options = $this->getServiceLocator()->get('admin_metrics_service')->getMetricsOptionsAll();
	...
	public function reportAction(){
		...
		$client_id      = (int) $this->params('id');
			...
			if($request->isPost()){
				...
				$metrics_value_options 		   = $this->getServiceLocator()->get('admin_metrics_service')->getMetricsOptions($metrics_type);
				$metrics_compare_value_options = $this->getServiceLocator()->get('admin_metrics_service')->getMetricsOptions($metrics_type_compare);
	...
	public function fetchCampaignsAction(){
	  ...
	  $campaigns = $this->getServiceLocator()->get('admin_campaign_service')->setClientId($client->getAdwordsClientId())->getCampaigns($type);
	...
	public function viewReportAction1(){
		...
		print_r($args);
		exit;*/
		$result = $this->getServiceLocator()->get('admin_campaign_service')->setClientId($client->getAdwordsClientId())
