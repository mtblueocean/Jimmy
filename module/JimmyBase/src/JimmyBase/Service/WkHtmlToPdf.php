 ...
 */
class WkHtmlToPdf extends EventProvider  implements ServiceManagerAwareInterface
{
    protected $local_path  = '/usr/local/bin/wkhtmltopdf';
    protected $server_path = '/usr/local/bin/wkhtmltopdf';
    protected $bin = null;//'/Applications/wkhtmltopdf';
    ...
    public function getCommand($filename)
    {
        // $request = $this->getServiceManager()->get('Request');
         ...
         //$httpHost = $request->getUri()->getHost();

         $config  = $this->getServiceManager()->get('Config');
         $baseUrl = $config['jimmy-config']['baseurl'];

         if(preg_match('/localhost/',$baseUrl))
             $this->bin = $this->local_path;
         else
             $this->bin = $this->server_path;
